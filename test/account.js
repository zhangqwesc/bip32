...
test('containsAddress', function (t) {
  let account = Account.fromJSON(f.neutered.json)
...
test('bip44', function (t) {
  let seed = Buffer.alloc(32, 0x03)
  let account = Account.bip44FromSeed(seed, 44, 0x00, 0)

  t.same(account.getChainAddress(0), '')
  t.same(account.getChainAddress(1), '')
  t.end()
})

test('discoverChain', function (t) {
  let account = Account.fromJSON(f.neutered.json)
  let before = account.getChainAddress(0)
  let after = account.getChain(0).clone().next()
...
test('getAllAddresses', function (t) {
  let account = blankAccount(f.neutered.json)
...
test('derive', function (t) {
  let neutered = Account.fromJSON(f.neutered.json)

  t.test('neutered node', function (t) {
    f.addresses.forEach(function (addresses, i) {
      addresses.forEach(function (address, j) {
        let actual = neutered.derive(address)
        let expected = f.neutered.children[i][j]

        t.equal(expected, actual.toBase58(), 'return a neutered node')
      })
    })

    let unknown = neutered.derive('mpFZW4A9QtRuSpuh9SmeW7RSzFE3TgB8Ko')
    t.equal(undefined, unknown, 'ignores unknown addresses')
    t.end()
  })

  let priv = Account.fromJSON(f.private.json)

  t.test('neutered node w/ escalation', function (t) {
    let privParents = priv.chains.map(function (x) { return x.__parent })

    f.addresses.forEach(function (addresses, i) {
      addresses.forEach(function (address, j) {
        let actual = neutered.derive(address, privParents)
        let expected = f.private.children[i][j]
  ...
  t.test('private node', function (t) {
    f.addresses.forEach(function (addresses, i) {
      addresses.forEach(function (address, j) {
        let actual = priv.derive(address)
        let expected = f.private.children[i][j]
